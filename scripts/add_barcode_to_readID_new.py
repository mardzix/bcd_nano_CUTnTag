...
class bcdCT:
    ...
    def create_out_handles(self,stack):
        ...
        if self.single_cell:
            self.path_out  = {barcode: {read: "{0}/barcode_{2}/{1}_{2}_{3}.fastq.gz".format(self.out_prefix,self.name,barcode,read) for read in out_reads} for barcode in self.picked_barcodes}
        ...
        for read1,read2,read3 in exp:
            ...
            if spacer_hit:
                ...
                if len(read2.sequence) < 16:
                  statistics["barcode_too_short"] += 1
                  continue
